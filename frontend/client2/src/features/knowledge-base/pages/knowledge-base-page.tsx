...
export function KnowledgeBasePage() {
  ...
  const hasPendingFiles = useMemo(() => selectedFiles.length > 0, [selectedFiles.length])
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToDocumentSources(
      user.id,
      (files) => {
        setSources(files)
        setIsLoading(false)
    ...
    <div className="space-y-8">
      <section className="rounded-2xl border border-emerald-100 bg-white p-8 shadow-sm">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Knowledge Base</h1>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-slate-500">{sources.length} total</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-3">
            {sources.map((source) => {
              ...
              return (
                <article key={source.id} className="rounded-xl border border-emerald-100 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <p className="text-sm font-semibold text-slate-900">{source.name}</p>
                      <p className="text-xs text-slate-500">
                        Uploaded: {formatDate(source.createdAt)} | Size: {formatSize(source.sizeBytes)}
                      </p>
                      <a
                        href={source.url}
                        target="_blank"
