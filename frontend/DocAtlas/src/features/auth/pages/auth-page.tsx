...
function InputField({
  ...
  return (
    <div className="space-y-2">
      <label htmlFor={id} className="text-[11px] font-semibold uppercase tracking-[0.12em] text-emerald-900/80 sm:text-xs">
        {label}
      </label>
      <input
        id={id}
      ...
      <div className="mx-auto grid w-full max-w-7xl overflow-hidden rounded-2xl border border-black/10 bg-white shadow-[0_24px_64px_rgba(0,0,0,0.08)] md:max-h-[calc(100dvh-9.5rem)] md:min-h-0 md:grid-cols-2">
        <aside className="relative flex flex-col items-center justify-center gap-4 overflow-y-auto bg-emerald-950 px-6 py-10 text-center text-emerald-50 sm:px-8 sm:py-12 md:gap-3 md:px-8 md:py-8 lg:px-12 lg:py-12">
          ...
          <div className="relative flex justify-center">
            <img src={docAtlasLogo} alt="DocAtlas logo" className="size-28 object-contain sm:size-36 md:size-44 lg:size-56 xl:size-64" />
          </div>

          <div className="relative space-y-3 md:space-y-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-300 sm:text-sm sm:tracking-[0.2em]">{t('auth.adminPlatform')}</p>
            <h2 className="text-2xl font-semibold leading-tight tracking-tight sm:text-3xl md:text-4xl">{t('auth.heroTitle')}</h2>
            <p className="mx-auto max-w-sm text-sm leading-6 text-emerald-100/75 sm:text-base sm:leading-7">{t('auth.heroDescription')}</p>
          </div>
